...
length(unique(updated_All_event_df[,"ID"])) #41375
...
################################################################################ 
analysis_ID <- unique(intersect(Comb_perday_df[,"study_id"],Valid_Month_df[,"ID"])) # 27722
analysis_ID_udpated <- unique(intersect(analysis_ID,Valid_Month_df[,"ID"])) # 27722
length(analysis_ID_udpated)
