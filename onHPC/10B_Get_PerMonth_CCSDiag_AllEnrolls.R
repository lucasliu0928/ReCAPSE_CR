...
#reformat 
diag_grp_df[,"TYPE"] <- "DIAG_ICD"  #change code type name
diag_grp_df[,"CCS_CATEGORY"] <- paste0("CCS_DIAG_",diag_grp_df[,"CCS_CATEGORY"]) #add group prefix
colnames(diag_grp_df)[which(colnames(diag_grp_df) == "CCS_CATEGORY")] <- "CCS_DIAG" #change group type column name
...
foreach (i = 1: length(analysis_IDs)) %dopar% {
  ...
  if (ncol(curr_perMonth_df) > 4){ #Make sure there is any code left in the df, if not, this pts should be excluded for final
    ...
    #unique codes
    unique_codes_df   <- get_codes_func(code_names,"DIAG_ICD")
    ...
    if (is.null(unique_codes_df) == T){ #if does not have any code in this type
      ...
      unique_grps_df <- data.frame("unique_grps" = "NONE") #craete an emtpy unique grp df
        ...
        unique_codes_df[,"GRPS"] <- find_listofcode_grp_func(unique_codes_df,grp_type,diag_grp_df)
