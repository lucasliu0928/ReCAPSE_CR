...
data_dir <- "/recapse/intermediate_data/perDay_PerPatientData/"
outdir <- "/recapse/intermediate_data/perDay_ValidMonth_PerPatientData/"

# #local
# valid_month_dir <- "/Users/lucasliu/Desktop/intermediate_data/"
# data_dir <- "/Users/lucasliu/Desktop/intermediate_data/perDay_PerPatientData/"
# outdir <- "/Users/lucasliu/Desktop/intermediate_data/perDay_ValidMonth_PerPatientData/"
...
#Load Valid months
############################################################
valid_month_df <- read.xlsx(paste0(valid_month_dir,"valid_month_df.xlsx"),sheet = 1)
...
foreach (i = 1: length(perDay_files)) %dopar% {
  ...
  if (nrow(curr_valid_month_df) > 0){
    start <- ymd(curr_valid_month_df$Valid_Start)
    end <- ymd(curr_valid_month_df$Valid_End)
  }
  
  filtered_df <- curr_perDay_df[which(ymd(curr_perDay_df$claims_date) >= start & ymd(curr_perDay_df$claims_date) <=end ),]
