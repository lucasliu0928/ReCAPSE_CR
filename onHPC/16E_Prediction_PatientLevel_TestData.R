...
for (ds_index in 0:10){
  ...
  for (model in model_list){
    ...
    #2.Get acutal SBCE and SBCE month
    sbce_df <- get_pt_actual_sbcelabel_month(ds_pred_df,pts_level_char_df)
    
    #3. Get  predictioned month and label by the first month that the prediction probability is greater or equal to threholds
    thres_list <- seq(1,9,1)
    method_name <- "OneMonth_GT_Threshold"
    pt_pred_df1 <- get_allpt_level_pred(ds_pred_df,sbce_df,thres_list,method_name)
    write.csv(pt_pred_df1,paste0(outdir, ds_out,model,"_",method_name,"_patientlevel_pred_tb.csv"))
    
    #4. Get predictied month and predicted label by persisit 3 months
    thres_list <- seq(1,9,1)
