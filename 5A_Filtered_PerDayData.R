#Descrption: This script keep claims per day data are in valid month
...
################################################################################ 
#Get valid per day data for each patient
################################################################################ 
analysis_ID <- unique(intersect(unique(Comb_perday_df[,"study_id"]),unique(Valid_Month_df[,"ID"]))) #26840
...
for (i in 1:length(analysis_ID)){
  ...
  n_valid_month[i] <- difftime(max_month,min_month,units = "days") #use max - min, cuz the month in curr_enroll_months might not be consectutive
